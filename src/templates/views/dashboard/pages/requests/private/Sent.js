...
const Sent = () => {
  ...
  const { isLoading,  data } = useGetApi(
    `Requests`,
    {
      request_type: ['private', 'sent'],
      company_id: user?.id,
      user_type: user?.type
    },
    `requestLists_private_sent_${user?.id}`,
    {
      enabled: !!user?.id,
  ...
  return (
    <Row>
      <SeoGenerator
        title="Dashboard | Requests - private - Sent"
      />

      <Col span={ 24 }>
        <DashboardContentHeader
          page={ "private sent requests" }
                          ...
                          <Row gutter={10}>
                            <Col flex="1 1" className="--details">
                              <div className="buttonSection">
                                <span className="--sendTo--title">{ t('send_request_to') }: </span>
                                <span className="--sendTo"> {request?.send_to}</span>
                              </div>
                            </Col>
                ...
                selectedRequest?.request_id ?
                  <Row className="row-cols-1" gutter={[0, 30]}>
                    ...
                    <Col>
                      <Row gutter={16}>
                        <Col span={12} className="--variable text-47">
                          { t(__('send request to')) }:
                        </Col>
                        <Col span={12} className="--value text-92">
                          { selectedRequest?.send_to }
                        </Col>
                      </Row>
                    </Col>
